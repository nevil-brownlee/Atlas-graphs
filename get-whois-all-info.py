...
#
# get-whois-allinfo.py:  Looks up whois info for ASNs in asns-all file
#
...
print("Will read asns-all file %s" % all_asns_fn)
aaf = open(all_asns_fn, 'r')

asn_d = {}  # Dictionary of ASN objects, key 

for line in aaf:  # Get dictionary of ASNs, with counts for each
    addr, asn = line.strip().split()
    #print("%s = %s" % (addr, asn))
    if asn in asn_d:
        asn_d[asn].count += 1
    else:
...
whoisf = open(whois_fn, "w", encoding='utf-8')
print("Will write whois-all info file %s" % whoisf)
