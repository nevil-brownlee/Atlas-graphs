...
reqd_ymds = [];  reqd_msms = []
pp_names = "m! y! a mxd= mntr= sd! dir!"  # indeces 0 to 6
...
for n,ix in enumerate(pp_ix):
    if ix == 0:    # m  (50xx) msm_ids
        ...
        subdirs = pp_values[n]
    elif ix == 6:  # dir    specify dir for SVG images (and html output)
        svg_dir = pp_values[n][0]
        print("svg_dir = %s" % svg_dir)
    else:
        exit()
if len(reqd_ymds) == 0:
...
if len(reqd_msms) == 0:
    reqd_msms = [c.msm_id]
if len(subdirs) != 1 and not svg_dir:
...
if svg_dir:
    print("Use SVG images from %s" % svg_dir)
...
for msm_id in reqd_msms:
    ...
    if svg_dir:
        dd_dgs_stem = "%s/%s/%s" % (svg_dir, subdirs[0], dgs_stem)
        draw_dir = "%s/%s" % (svg_dir, subdirs[0])
        slides_fn = "%s/%s-slides.html"  % (draw_dir,dgs_stem)
    else:
    ...
    print("draw_dir = %s" % draw_dir)
    print("slides_fn = %s" % slides_fn)

    bn_lo = 9999;  bn_hi = -1
    gg_str = dd_dgs_stem + "*.svg"
    print("gg_str >%s<" % gg_str)
